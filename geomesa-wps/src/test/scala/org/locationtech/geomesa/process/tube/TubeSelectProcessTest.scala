...
import org.locationtech.geomesa.process.tube.{TubeSelectProcess, NoGapFill, DEFAULT_DTG_FIELD}
...
class TubeSelectProcessTest extends Specification {
  ...
  val dtgField = org.locationtech.geomesa.process.tube.DEFAULT_DTG_FIELD
    ...
    "should do a simple tube with geo interpolation" in {
      ...
      List("a", "b").foreach { name =>
        List(1, 2, 3, 4).zip(List(45, 46, 47, 48)).foreach { case (i, lat) =>
          ...
          sf.setAttribute(org.locationtech.geomesa.process.tube.DEFAULT_DTG_FIELD, new DateTime("2011-01-01T00:00:00Z", DateTimeZone.UTC).toDate)
    ...
    "should do a simple tube with geo + time interpolation" in {
      ...
      List("c").foreach { name =>
        List(1, 2, 3, 4).zip(List(45, 46, 47, 48)).foreach { case (i, lat) =>
          val sf = AvroSimpleFeatureFactory.buildAvroFeature(sft, List(), name + i.toString)
          sf.setDefaultGeometry(WKTUtils.read(f"POINT($lat%d $lat%d)"))
          sf.setAttribute(org.locationtech.geomesa.process.tube.DEFAULT_DTG_FIELD, new DateTime("2011-01-02T00:00:00Z", DateTimeZone.UTC).toDate)
          sf.setAttribute("type", name)
          sf.getUserData()(Hints.USE_PROVIDED_FID) = java.lang.Boolean.TRUE
  ...
  "TubeSelect" should {
    "should handle all geometries" in {
      ...
      bLine.setAttribute(org.locationtech.geomesa.process.tube.DEFAULT_DTG_FIELD, new DateTime("2011-01-01T00:00:00Z", DateTimeZone.UTC).toDate)
      ...
      bPoly.setAttribute(org.locationtech.geomesa.process.tube.DEFAULT_DTG_FIELD, new DateTime("2011-01-01T00:00:00Z", DateTimeZone.UTC).toDate)
      ...
      aLine.setAttribute(org.locationtech.geomesa.process.tube.DEFAULT_DTG_FIELD, new DateTime("2011-01-01T00:00:00Z", DateTimeZone.UTC).toDate)
