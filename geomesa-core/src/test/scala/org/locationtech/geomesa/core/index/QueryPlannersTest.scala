...
class QueryPlannersTest extends Specification {
  ...
  "QueryPlanner" should {
    ...
    "return full ranges for unspecified dates " in {
      ...
      mm.getKeyPlan(SpatialFilter(ghPoly), ExplainLogging) must be equalTo KeyRange("01", "12")
      ss.getKeyPlan(SpatialFilter(ghPoly), ExplainLogging) must be equalTo KeyRange("00", "59")
      mmdd.getKeyPlan(SpatialFilter(ghPoly), ExplainLogging) must be equalTo KeyRange("01-01", "12-31")
    }
    ...
    "return appropriate ranges for date ranges" in {
      ...
      mm.getKeyPlan(DateRangeFilter(dt1, dt2), ExplainLogging) must be equalTo KeyRangeTiered("03", "10")
      ss.getKeyPlan(DateRangeFilter(dt1, dt2), ExplainLogging) must be equalTo KeyRangeTiered("00", "59")
      mm.getKeyPlan(DateRangeFilter(dt3, dt1), ExplainLogging) must be equalTo KeyRangeTiered("01", "12")
      mm.getKeyPlan(DateRangeFilter(dt1, dt4), ExplainLogging) must be equalTo KeyRangeTiered("03", "03")
      mm.getKeyPlan(DateRangeFilter(dt4, dt5), ExplainLogging) must be equalTo KeyRangeTiered("03", "09")
    }
    ...
    "return appropriate regexes for regex" in {
      ...
      val kp = cp.getKeyPlan(SpatialFilter(poly), ExplainLogging)
