...
class IndexSchemaTest extends Specification {
  ...
  "SpatioTemporalIndexSchema" should {
    "parse a valid string" in {
      val schema = IndexSchema.buildKeyEncoder(
        "%~#s%foo#cstr%99#r::%~#s%0,4#gh::%~#s%4,3#gh%15#id", dummyEncoder)
    ...
    "allow extra elements inside the column qualifier" in {
      val schema = Try(IndexSchema.buildKeyEncoder(
        "%~#s%foo#cstr%0,1#gh%99#r::%~#s%1,5#gh::%~#s%9#r%ColQ#cstr%15#id%5,2#gh", dummyEncoder))
      schema must beAFailedTry
    }

    "complain when there are extra elements at the end" in {
      val schema = Try(IndexSchema.buildKeyEncoder(
        "%~#s%foo#cstr%0,1#gh%99#r::%~#s%1,5#gh::%~#s%15#id%5,2#gh", dummyEncoder))
      schema must beAFailedTry
    }

    "allow index/data flags" in {
      val schema = IndexSchema.buildKeyEncoder(
        "%~#s%#i%foo#cstr%99#r::%~#s%0,4#gh::%~#s%4,3#gh%15#id", dummyEncoder)
  ...
  val encoder = IndexSchema.buildKeyEncoder(schemaEncoding, dummyEncoder)
  ...
  "single-point (-78.4953560 38.0752150)" should {
    "encode to 1 index row" in {
      ...
      val indexEntries = encoder.encode(item, "").toList
  ...
  "index-entry encoded and decoder" should {
    "encode and decode round-trip properly using a custom date-time field name" in {
      ...
      val schema = "%~#s%99#r%TEST#cstr%0,3#gh%yyyyMMdd#d::%~#s%3,2#gh::%~#s%#id"
      val encoder = IndexSchema.buildKeyEncoder(schema, customEncoder)
      val decoder = IndexSchema.getIndexEntryDecoder(schema)

      val encodedKVs = encoder.encode(entry, "")
      ...
      val decoded = encodedKVs.head match {
        case (key, value) => decoder.decode(key)
      }
